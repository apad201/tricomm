...
ggplot(modelParams) +
  ...
  geom_point(aes(x=time,y=modelAccuracySmV), color = "blue") +
  geom_point(aes(x=time, y=modelAccuracyV), color = "red") +
  ylim(-1,1)
```
...
stepsDataPredSmall <- stepsDataPredSmall %>%
  mutate(time = substr(time, 2,8)) %>%
  mutate(time = as.numeric(time)) %>%
  mutate(timeOfDay = case_when(
    time < 8.0 ~ "earlyAM",
